...
import { SalesChain } from "@/components/sales-chain";
import { cropSubs } from "@/lib/substitutes";
import { layersFile, marketFile, nassFarm, processFile } from "@/lib/data";
...
function BoardPage() {
  ...
  const [formId, setFormId] = useState("fresh");
  const { cad, num, tag } = useDeskMoney();
  const forms = cropSubs(cmd).forms;
  const formSafe = forms.some((f) => f.id === formId) ? formId : forms[0]?.id || "fresh";
  ...
  const barData = regions
    ...
    .map((r) => ({
      city: r.centre,
      wholesale: num(r.snap!.p50!),
      farm: farm != null ? num(farm) : undefined,
    }));
  ...
  return (
    <div className="min-h-dvh bg-paper text-ink">
      <SiteHeader kicker="Current public prints down the sales chain — fresh, frozen, dried, juice, paste." />

      <section className="px-4 pt-6 sm:px-9">
        <h1 className="font-display text-3xl leading-tight sm:text-4xl">
          What does this actually print right now?
        </h1>
        <p className="mt-2 max-w-2xl text-sm text-ink-soft">
          {processFile.thesis} {nassCount} USDA fresh farm series. {both} also print Canadian wholesale.
        </p>
        <div className="mt-2">
          <Freshness />
        </div>
      ...
      <section className="px-4 py-6 sm:px-9">
        <h2 className="font-display text-2xl">{cmd} · sales chain</h2>
        <p className="mt-1 text-sm text-ink-soft">
          Pick a form. Fresh wholesale is the current destination ask. Frozen, dried, and juice are processor or grocery tapes — more comparable, lagging, and a different SKU.
        </p>
        <div className="mt-4">
          <SalesChain cmd={cmd} formId={formSafe} onForm={setFormId} />
        </div>
      </section>
      ...
      <section className="grid gap-4 px-4 py-6 sm:px-9 lg:grid-cols-2">
        <div className="min-w-0 rounded-xl border border-line bg-cream p-4">
          ...
          <p className="mt-1 text-xs text-ink-soft">
            AAFC destination mids, {tag}. The bar is not a US FOB. Farm line is US NASS national, converted.
          </p>
          {barData.length ? (
            <div className="mt-4 h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={barData}>
                  ...
                  <YAxis tick={{ fontSize: 11, fill: "var(--color-ink-soft)" }} tickFormatter={(v: number) => `$${v}`} width={48} />
                  <Tooltip
          ...
          <ul className="mt-3 grid grid-cols-2 gap-2 text-sm">
            {regions.map((r) => (
              <li key={r.centre} className="rounded-lg border border-line p-2">
                ...
                <div className="font-medium">{r.snap?.p50 != null ? `${cad(r.snap.p50)} ${tag}` : "—"}</div>
        ...
        <div className="min-w-0 rounded-xl border border-line bg-cream p-4">
          ...
          <p className="mt-1 text-xs text-ink-soft">
            Latest {farm != null ? `${cad(farm)} ${tag}` : "—"}
            {series?.latest ? ` · ${series.latest.d.slice(0, 7)} · $${fmt(series.latest.usdLb)}/lb USD native` : " · no NASS series"}
          </p>
      ...
      <section className="px-4 pb-6 sm:px-9">
        <h2 className="font-display text-xl">Own vs rent on this chain</h2>
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="text-ink-soft">
                ...
                <th className="px-3 py-2 font-medium">Latest farm {tag}</th>
              </tr>
            </thead>
            <tbody>
              {catalog.map((c) => {
                ...
                return (
                  <tr key={c.label} className="border-t border-line">
                    ...
                    <td className="px-3 py-2">{s?.latest ? cad(s.latest.cadKg) : "—"}</td>
                  </tr>
                );
              })}
            </tbody>
